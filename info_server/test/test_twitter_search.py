...
class MockChannel:
    ...
    async def receive_from(self, group):
        ...
        if self.call_count == 0:
            self.call_count += 1
            return 1, "bob", "search_user"  # 假设搜索关键词为"bob"
        if self.call_count == 1:
            self.call_count += 1
            return 2, "Bob", "search_tweet"  # 假设搜索关键词为"bob"
        # 后续调用返回退出指令
        else:
    ...
    async def send_to(self, group, message):
        ...
        if self.call_count == 1:
            # 验证搜索成功且找到至少一个匹配用户
            assert message[1]["success"] is True, "Search should be successful"
            assert len(
                message[1]["users"]) > 0, "Should find at least one user"
            # 你可以添加更多的断言来验证返回的用户信息是否正确
            assert message[1]["users"][0]["user_name"] == \
                   "user2", "The first matching user should be 'user2'"
        if self.call_count == 2:
            assert message[1]["success"] is True, "Search should be successful"
            assert len(
                message[1]["tweets"]) > 0, "Should find at least one tweet"
            assert message[1]["tweets"][0]["content"] == "Bob's first tweet!"
...
async def test_search_user(setup_twitter):
    try:
        ...
        cursor = conn.cursor()
        users_info = [(1, "user1", "Alice", "Bio of Alice",
                       "2023-01-01 12:00:00", 10, 5),
                      (2, "user2", "Bob", "Bio of Bob", "2023-01-02 12:00:00",
                       15, 8),
                      (3, "user3", "Charlie", "Bio of Charlie",
